...
const RegisterInputs = () => {
    const URL_BASE = "http://localhost:3001/api/clients";
    const user = useId();
    const email = useId();
    const pass = useId();
    ...
    const loginUser = ({ id, apiKey }) => {
        dispatch(saveSessionId(id));
        dispatch(saveSessionToken(apiKey));
    ...
    return (
        <div className='flex justify-center mt-12'>
            <form className="flex flex-col items-center justify-center self-end" onSubmit={handleRegister}>
                <div className="mb-2 w-1/2">
                    <label htmlFor={user} className="block text-gray-700 text-sm font-medium mb-1">Nombre</label>
                    <input type="text" className="w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 input-transparente" ref={userField} id={user} />
                </div>
                <div className="mb-2 w-1/2">
                ...
                <button type="submit" className="px-4 py-2 bg-blue-500 text-white font-semibold rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">Registrarme</button>
