...
import { Box, Typography, Button, Avatar, Rating, Container } from '@mui/material';
...
const ProductDetail = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <Header store />
      <Container className="container mx-auto p-4 mt-[5rem]" style={{ flex: '1' }}>
        <Box className="bg-white p-6 rounded shadow-md">
          <Box className="flex flex-col items-center">
            <Avatar
              src={product.image}
              alt={product.name}
              sx={{ width: 300, height: 300, marginBottom: 2, borderRadius: '8px' }}
              variant="square"
            />
            <Typography variant="h4" className="font-bold mb-2">{product.name}</Typography>
            <Typography variant="h5" className="text-colors-1 font-bold mb-2">${product.price}</Typography>
            <Typography variant="body1" className="mb-4">{product.description}</Typography>
            <Typography variant="body2" className="mb-4">Stock: {product.stock}</Typography>
            <Typography variant="body2" className="mb-4">Categoría: {category.name}</Typography>
            ...
            >
              {isFavorite(product.id) ? 'Eliminar de Favoritos' : 'Agregar a Favoritos'}
            </Button>
          </Box>
        </Box>
      </Container>
