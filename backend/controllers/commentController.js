...
const createComment = async (req, res) => {
    ...
    try {
        ...
        if (!user || !post) {
            return res.status(400).json({ message: 'El usuario o el post especificado no existe' });
        }
        ...
        const newComment = await Comment.create({ userId, postId, content, photo: imageUrl });
...
const updateComment = async (req, res) => {
    ...
    try {
        ...
        const [updated] = await Comment.update({ userId, postId, content, photo: imageUrl }, { where: { id } });
        if (updated === 0) {
            return res.status(404).json({ message: `Comentario con id ${id} no encontrado` });
        }
        const updatedComment = await Comment.findByPk(id);
        res.json({ message: 'Comentario actualizado con éxito', data: updatedComment });
    } catch (error) {
...
export { getCommentsByPost, createComment, updateComment, deleteComment, upload };